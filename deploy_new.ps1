...
$UnrealPakDir = "C:\dev\stalker2\unrealpak-main"
$UnrealPakExe = Join-Path $UnrealPakDir "UnrealPak.exe"
...
function Deploy-Mod([string]$Name) {
    ...
    # Create filelist.txt for UnrealPak (Mount Point: ../../../)
    $FileListPath = Join-Path $UnrealPakDir "filelist.txt"
    # Ensure source path ends with \* and target is the triple-dot root with a trailing backslash
    $FileListItem = "`"$PackDir\*.*`" `"../../../*.*`""
    $FileListItem | Out-File -FilePath $FileListPath -Encoding ascii -Force

    # Execute UnrealPak directly
    Push-Location $UnrealPakDir
    try {
        & $UnrealPakExe "$SourceFile" -create="$FileListPath" -compress
    } finally {
        Pop-Location
    }
...
if ([string]::IsNullOrWhiteSpace($ModName)) {
    ...
    while (-not $Finished) {
        Clear-Host
        Write-Host "`n  === S.T.A.L.K.E.R. 2 MOD DEPLOYER (NEW) ===" -ForegroundColor DarkCyan
